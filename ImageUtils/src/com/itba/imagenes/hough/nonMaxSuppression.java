...
public class nonMaxSuppression {
	...
	public void init(int[] magnitudeIn, double[] directionIn, int widthIn,
			int heightIn) {
		width = widthIn;
		height = heightIn;
		...
		for (int x = 0; x < width; x++) {
			...
			for (int y = 0; y < height; y++) {
				if ((magnitude[y * width + x] & 0xff) > 0) {
					...
					int x1 = (int) Math
							.ceil((Math.cos(angle + Math.PI / 8) * roottwo) - 0.5);
					int y1 = (int) Math
							.ceil((-Math.sin(angle + Math.PI / 8) * roottwo) - 0.5);
					int x2 = (int) Math
							.ceil((Math.cos(angle - Math.PI / 8) * roottwo) - 0.5);
					int y2 = (int) Math
							.ceil((-Math.sin(angle - Math.PI / 8) * roottwo) - 0.5);

					double M1 = (magnitude[(y + y1) * width + (x + x1)] & 0xff
							+ magnitude[(y + y2) * width + (x + x2)] & 0xff) / 2;
					...
					double M2 = (magnitude[(y + y1) * width + (x + x1)] & 0xff
							+ magnitude[(y + y2) * width + (x + x2)] & 0xff) / 2;

					if ((Mint > M1) && (Mint >= M2)) {
						output[y * width + x] = 0xff000000 | (Mint << 16
								| Mint << 8 | Mint);
					} else {
